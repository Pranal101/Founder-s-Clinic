...
//         console.log("exp response", response.data.profile.workExperience);
//         if (experienceData) {
...
const Experiences = () => {
  ...
  const handleSaveNewExperience = async (e) => {
    ...
    try {
      ...
      // Debugging log to view the payload being sent
  ...
  const handleDeleteExperience = async (experienceId) => {
    try {
      ...
      await axios.patch(
        "https://founders-clinic-backend.onrender.com/api/user/profile/intern-experience",
        {
          userId: user.uid,
          experienceId,
        },
